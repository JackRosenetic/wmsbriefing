...
const SLACK_USER_ID = 'U0A0V9RHZSN';
...
async function main() {
  ...
  try {
    ...
    console.warn('Re-auth failed, proceeding with original cookie');
  }
  ...
  const rows = Array.isArray(forecast) ? forecast : (forecast?.rows || forecast?.hampers || []);
  const allProducts = [...rows].sort((a, b) => (a.daysCover ?? 9999) - (b.daysCover ?? 9999)).slice(0, 5);
  const urgentLines = allProducts.length
    ? allProducts.map(p => `• ${p.productName} - Days of Cover (${Math.round((p.daysCover ?? 0) * 10) / 10}d) Recommended Send ${p.recommendedSend}`).join('\n')
    : '(data unavailable)';
